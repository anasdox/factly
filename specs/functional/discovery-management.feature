Feature: Discovery Management
  ...
  Scenario: Create a new discovery
    ...
    And the title, goal, and date fields are empty

  @fsid:FS-SaveNewDiscovery
  Scenario: Save a new discovery
    Given the Discovery modal is open in "add" mode
    When the Analyst fills in the title, goal, and date
    And clicks "Add"
    Then the discovery is created with the provided title, goal, and date
  ...
  Scenario: Edit an existing discovery
    ...
    And the title, goal, and date fields are pre-filled with the current values
  ...
  Scenario: Save an edited discovery
    ...
    When the Analyst modifies the title, goal, or date
    And clicks "Save"
