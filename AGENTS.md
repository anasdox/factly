...
1. For each functional spec, write acceptance tests in `tests/blackbox/`.
...
2. Run all acceptance tests in `tests/blackbox`.
...
6. Re-run all acceptance tests in `tests/blackbox`, then run full tests and ensure CI stays green.
...
- Acceptance tests (backend/API/domain): `tests/blackbox/`
- Browser E2E acceptance tests (UI workflows): `tests/e2e/`
- Applications source codes: `apps/<app-name>/`


Traceability expectation:
- roadmap feature -> implementation plan slice -> FSID -> TSID -> executable tests (`tests/blackbox/` and/or `tests/e2e/`) must be reviewable end-to-end.
...
- [ ] One executable test file per feature in `tests/blackbox/` and/or `tests/e2e/` (depending on backend vs browser behavior)
